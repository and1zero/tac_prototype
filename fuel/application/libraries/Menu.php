...
class Menu {
    ...
    protected function _render($root_items) {
        switch ($this->render_type) {
            ...
            case 'array':
                ...
                break;
            case 'dropdown':
                $output = $this->_render_dropdown($root_items);
                break;
            default:
    ...
    protected function _render_basic($menu, $level = -1) {
        ...
        if (!empty($menu) AND (isset($this->depth) AND $level < $this->depth) OR !isset($this->depth)) {
            ...
            if (!empty($menu)) {
                ...
                if (!empty($this->container_tag_id) AND $level == -1)
                    $str .= " id=\"" . $this->container_tag_id . "\"";
                if (!empty($this->container_tag_class) AND $level == -1)
                    $str .= " class=\"" . $this->container_tag_class . "\"";
                if (!empty($this->container_tag))
                    $str .= ">\n";
                ...
                if (!empty($this->container_tag))
                    $str .= str_repeat("\t", $level) . "</" . $this->container_tag . ">\n" . str_repeat("\t", $level);
            }
        }
        return $str;
    }
    ...
    protected function _render_collabsible($menu, $level = 0) {
        ...
        if (!empty($menu)) {
            ...
            if (!empty($this->container_tag_id) AND $level == 0)
                $str .= " id=\"" . $this->container_tag_id . "\"";
     ...
     * @return	string
     * 
     * @author andi.susanto@theadventus.com
     * @since version 1.0.0
     */
    protected function _render_dropdown($menu, $level = 0, $parent_id = '') {
        // filter out hidden ones first. Need to do in seperate loop in case there is a hidden on e at the end
        $menu = $this->_filter_hidden($menu);
        ...
        if (!empty($menu)) {
            ...
            if (!empty($this->container_tag_class) AND $level == 0)
                $str .= " class=\"" . $this->container_tag_class . "\"";
            if ($level == 0)
                $str .= ' role="navigation" '; else
                $str .= ' aria-labelledby="' . $parent_id . '" class="dropdown-menu" role="menu" ';
            if (!empty($this->container_tag))
                $str .= ">\n";
            ...
            foreach ($menu as $key => $val) {
                ...
                if ($label) {
                    ...
                    if (!empty($this->item_tag)) {
                        $attributes = '';
                        $str .= "<" . $this->item_tag;
                        $str .= $this->_get_li_classes($key, $val['id'], $level, ($i == (count($menu) - 1)), $subitems);
                        ...
                        // Set dropdown if it has children
                        if (!empty($subitems)) {
                            $str .= ' class="dropdown" ';
                            $label .= '<b class="caret"></b>';
                            $sub_id = $val['nav_key'] . $val['id'];
                            $attributes .= ' id="' . $sub_id . '" class="dropdown-toggle" data-toggle="dropdown" role="button" data-target="#" ';
                        }

                        $str .= '>';
                    }

                    $str .= anchor($val['location'], $label, $val['attributes'] . $attributes);
                    if (!empty($subitems)) {
                        $str .= $this->_render_dropdown($subitems, $level, $sub_id);
                    }
                    if (!empty($this->item_tag)) {
                        $str .= "</" . $this->item_tag . ">\n";
                    }
    ...
    protected function _render_breadcrumb($menu) {
        ...
        if (!empty($str)) {
            ...
            if (!empty($this->container_tag_id))
                $return .= " id=\"" . $this->container_tag_id . "\"";
            if (!empty($this->container_tag_class))
                $return .= " class=\"" . $this->container_tag_class . "\"";
            if (!empty($this->container_tag))
                $return .= ">\n";
    ...
    protected function _render_page_title($menu) {
        ...
        if ($this->order == 'desc') {
            ...
            if (($num >= 0 AND !empty($home_link)) OR (empty($home_link) AND $num > 0))
                $str .= $this->delimiter;
     ...
     * @param	int current item index
     * @param	boolean whether the item is the last in the list
     * @param   array the list of submenus listed
     * @return	string
     */
    protected function _get_li_classes($val, $level, $i, $is_last = FALSE, $subitems = array()) {
        $str = '';
        $css_classes = array();
        $active = (is_array($val)) ? $val['id'] : $val;

        if (!empty($this->first_class) AND $i == 0)
            $css_classes[] = $this->first_class;
        if (!empty($this->last_class) AND $is_last)
            $css_classes[] = $this->last_class;
        ...
        // Adding dropdown class if subitems is not empty
        if (!empty($subitems)) {
    ...
    public function get_items_in_path($active, $first_time = TRUE) {
        ...
        if (!in_array($active, $active_items))
            $active_items[] = $active;
